...
#### lombok
通过添加注解的方法，不需要为类编写getter或equal方法
![alt text](assets/学习/image.png)

### Maven
在pom.xml文件中引入依赖，可以自动让Edit Starters生成
引入新的依赖之后需要重新加载pom.xml文件方可生效
...
### Lambda表达式
rectangles.sort(Comparator.comparingDouble(r -> r.y));
对于每一个长方形对象r，取出r的y作为排序的依据

### List
list.sort(Comparator.comparingDouble(r->r.y)); //sort方法
list.add(index, element); // 在指定位置插入元素，如果插入在前面的位置，则之后的元素会自动往后移一位
list.add(element); // 默认在list最后插入位置
